  ...
  return (
    <article className="task-card" aria-label={title} aria-busy={busy}>
      ...
      <div className="task-actions">
        ...
        <button onClick={() => { if (window.confirm('¿Eliminar esta tarea?')) onEliminar(id); }} disabled={busy} className="btn-eliminar" aria-label={`Eliminar ${title}`} title="Eliminar tarea"><svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M6 7l1 13h10l1-13M10 10v7m4-7v7" /></svg></button>
      </div>
    </article>
  );
}
